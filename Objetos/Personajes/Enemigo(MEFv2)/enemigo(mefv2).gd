...
onready var filtro = [self, $sensores/pseudo_vision, $fisicas_balas]

export var velocidad = 50.0
export(float) var umbral = 5.0

onready var cerebro = $parabellum
onready var interactuador = $brazo_sup
...
func detener_movimiento(limpiar = false):
	if limpiar:
		camino = []
